      ...
      <div class="vs-component vs-con-table table-dark-inverted vs-table-primary con-tablex vs-table--content">
        <div class="table-responsive vs-con-tbody vs-table--tbody">
              ...
              <tr>
                <th>
                  <div class="vs-table-text justify-content-start">
                    ...
                    <div>{{ country[('palace' + capitalize(locale)) as keyof ReportRegionDto] }}</div>
                  </div>
                </th>
                <th>
                  <div class="vs-table-text justify-content-start">
                    <span class="mr-2 text-black-50">{{ t('period') }}: </span>
                    <div>{{ country?.from }} - {{ country?.to }}</div>
                  </div>
                </th>
              </tr>
            </thead>
          ...
          <table class="vs-table vs-table--tbody-table pb-5 top-table">
            ...
            <tbody>
              <tr
                v-for="(item, idx) in country.items"
                :key="item?.id"
...
const props = defineProps<{
  country: ReportRegionDto
  pagination?: TablePaginationType
}>()

const quantityTotal = computed(() => {
  return props.country.items.reduce((total, item) => total + (item?.quantity || 0), 0)
})

const sumTotal = computed(() => {
  return props.country.items.reduce((total, item) => total + (item?.sum || 0), 0)
})
</script>
