<template>
  <base-page-layout :title="t('reports.reportBySales')" :isLoading="loading">
    <template #filter>
      <v-card class="pa-4">
        <v-row>
          <v-col cols="12" md="3">
          ...
          <v-col cols="12" md="3" v-if="itemDate === 'interval'" class="base-select">
            <div class="base-date-input">
              ...
              <v-date-input :label="t('tickets.selectDate')" color="indigo" clearable multiple="range" density="compact"
                :value="dateView" prepend-icon="" prepend-inner-icon="mdi-calendar"
                @update:model-value="handleDateChange($event)" @click:clear="handleClear" :key="key" />
            </div>
          </v-col>
          ...
          <v-col cols="12" :md="itemDate === 'interval' ? 12 : 9" class="d-flex justify-end align-end">
...
import { getReportBySales, getReportBySalesExel } from '@/services/report-by-month'
import { getAllEventListBySaleDate, getCompanyAllList } from '@/services/organization/organization.service'
import { ref } from 'vue'
import type { AgentListHeader } from '@/services/report-by-month/model'
...
const fetchReportsByMonth = async () => {
  ...
  try {
    const { data } = await getReportBySales({ ...filter.value, ...dateRange })
...
const downloadExcel = async () => {
  ...
  try {
    const { data } = await getReportBySalesExel({ ...filter.value, ...dateRange })
    downloadBlobAsFile(data, 'ReportsBySales.xlsx')
  } catch (e) {
