<template>
  <base-title :title="t('user.edit')" :back="true" />
        ...
        <v-form ref="submitFormRef" @submit.prevent="handleSubmit">
          <v-card-item>
            <v-row>
              ...
              <v-col cols="12" md="3">
                <base-input v-model="form.password" :placeholder="t('login.password')"
                  :label="t('login.password')"></base-input>
              </v-col>
              <v-col cols="12" md="3">
                <base-select v-if="!form.isKassir" v-model="roleForm" :items="rolesList" multiple item-title="name"
                  item-value="id" :placeholder="t('user.roles')" :label="t('user.roles')"></base-select>
                <base-select v-if="form.isKassir" v-model="form.palaceId" :items="palaceList" item-title="text"
                  item-value="value" :label="t('events.palace')" :placeholder="t('events.palace')"></base-select>
              </v-col>
              <v-col cols="12" md="3">
            ...
            <v-row>
              <v-col cols="12" md="3" class="d-flex align-center">
                ...
                <base-button :loading="submitLoading" type="submit">{{ t('actions.save') }}</base-button>
              </v-col>
            </v-row>
          </v-card-item>
        </v-form>
...
import { CreateUserFormDto } from '@/services/administration/dto/admin.dto'
  ...
  if (validate && validate.valid) {
    ...
    try {
      ...
      if (form.value.isKassir === false) {
        const addedRoles = roleForm.value?.filter((role: number) => !oldUserRoles.value?.includes(role as never))
        const deletedRoles = oldUserRoles.value?.filter((role: number) => !roleForm.value?.includes(role as never))
        ...
        if (deletedRoles) {
          for (const roleId of deletedRoles) {
            const userRole = userRoles.value.find((r: any) => r.roleId === roleId)
        ...
        if (roleForm.value) {
          oldUserRoles.value = [...roleForm.value]
        }
      }
...
const loadById = async () => {
  try {
    const {
      data: { result }
    } = await getUserById(Number(router.params.id))
    form.value = result
...
const loadRoleById = async () => {
  try {
    ...
    roleForm.value = userRoles.value.map((userRole: any) => userRole.roleId)
    if (roleForm.value && roleForm.value.length > 0) {
      oldUserRoles.value = [...roleForm.value]
    }
  } catch (e) {
    toast.error(getErrorMessage(e))
...
onMounted(async () => {
  await Promise.all([loadData(), fetchData(), loadById()])
  await loadRoleById()
})
</script>
